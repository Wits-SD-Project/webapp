...
router.post("/upload", authenticate, async (req, res) => {
  ...
  try {
    ...
    snap.forEach((d) => {
      const f = d.data();
      if (
        f.name_lower === payload.name_lower &&
        f.type_lower === payload.type_lower
      ) {
        duplicate = true;
      }
    });
    ...
    const response = {
      ...
      facility: {
        ...
        location: saved.location, // ← add this
        ...
        ...(saved.features &&
          saved.features.length > 0 && { features: saved.features }),
      },
    };
...
router.put("/updateFacility/:id", authenticate, async (req, res) => {
  try {
    ...
    const { name, type, isOutdoors, availability, description, features } =
      req.body;
