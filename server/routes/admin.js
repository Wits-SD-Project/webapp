      ...
      if (booking.date && booking.slot) {
        ...
        try {
          ...
          if (!isNaN(bookingDateObject.getTime())) {
            ...
            const hourIndex = hourlyCounts.findIndex(
              (h) => h.hour === hourLabel
            );
            if (hourIndex !== -1) {
              ...
              console.warn(
                `Could not find hourIndex for hourLabel: ${hourLabel} from booking hour: ${hour}`
              );
            }
          } else {
            console.warn(
              `Could not parse date for booking ID ${doc.id}: ${bookingDateTimeString}`
            );
          }
        } catch (e) {
          console.warn(
            `Error processing slot for booking ID ${doc.id}: ${booking.slot}`,
            e
          );
        }
      }
