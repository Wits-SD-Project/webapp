    ...
    const fetchEvents = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_BASE_URL}/api/admin/upcoming`,
          {
            headers: {
        ...
        const lastSeenCount = parseInt(
          localStorage.getItem("lastSeenEventCount") || "0",
          10
        );
  ...
  useEffect(() => {
    const fetchUnreadNotifications = async () => {
      try {
        const token = await getAuthToken();
        const response = await fetch(
          `${process.env.REACT_APP_API_BASE_URL}/api/admin/unread-count`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        <main className="main-content">
          ...
          {showNewEventBanner && (
            <div className="new-event-banner">
              🎉 A new event has been posted! Check it out under "Upcoming
              Events".
            </div>
          )}
            ...
            <div className="card">
              ...
              <AnimatePresence mode="wait">
                <motion.p>
                  ...
                  upcoming event{events.length !== 1 ? "s" : ""}. Check them
                  out!
                </motion.p>
              </AnimatePresence>
              <button
