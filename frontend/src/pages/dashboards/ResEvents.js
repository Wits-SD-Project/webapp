...
export default function ResEvents() {
        ...
        try {
            ...
            fetchedEvents.forEach((event) => {
                if (event.facility && event.facility.id && event.facility.name) {
                    facilities.add(JSON.stringify({ id: event.facility.id, name: event.facility.name }));
                }
            });
    ...
    const filteredEvents = useMemo(() => {
        ...
        if (filterType === "upcoming") {
            currentEvents = currentEvents.filter((event) => event.startTime && isFuture(event.startTime));
        } else if (filterType === "past") {
            currentEvents = currentEvents.filter((event) => event.startTime && isPast(event.startTime));
        }
